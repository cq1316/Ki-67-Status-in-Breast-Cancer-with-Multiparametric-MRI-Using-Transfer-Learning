...
from single_stage_mr.utils.single_stage_mr import icc_feature_filter, get_feature, feature_select
...
if __name__ == "__main__":
    ...
    f = open(sys.argv[3])
    d = json.load(f)
    T2_icc_inds = d["feature_index"]
    f.close()
    f = open(sys.argv[4])
    d = json.load(f)
    D2_icc_inds = d["feature_index"]
    f.close()
    f = open(sys.argv[5])
    d = json.load(f)
    DWI_icc_inds = d["feature_index"]
    f.close()
    # 分数据集
    ...
    logging.info("get mr feature")
    # vgg特征提取器
    vgg_feature_selector = PoolVgg().cuda()
    # 混合mr
    get_feature(vgg_model=vgg_feature_selector,
                train_set=total_train_set,
                test_set=total_test_set,
                batch_size=16)
    icc_feature_filter(total_train_set + total_test_set, T2_icc_inds, D2_icc_inds, DWI_icc_inds)
    ...
    for feature_num in range(100, 3000, 40):
